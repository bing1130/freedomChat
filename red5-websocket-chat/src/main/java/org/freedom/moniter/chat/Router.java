...
public class Router {
	 ...
	 * @param message
	 */
	public void route(String path, HashMap endportMap) {
		log.info("Route to Shared Object: {} with {}", path, endportMap.keySet().toString());
		// get the shared object
		ISharedObject so = getSharedObject(path);
		if (so != null) {
			// set the message attribute
			so.setAttribute("endports", endportMap);
		} else {
	...
	public void route(String path, String message) {
		log.info("Route to Shared Object: {} with {}", path, message);
		// get the shared object
		ISharedObject so = getSharedObject(path);
		if (so != null) {
			// set the message attribute
	...
	public void route(IScope scope, String message) {
		...
		log.info("Route to WebSocket: {} with {}", path, message);
	...
	private final class SOListener implements ISharedObjectListener {
		...
		SOListener(Router router, IScope scope, String path) {
			log.info("ctor - path: {} scope: {}", path, scope);
			this.router = router;
			this.scope = scope;
			this.path = path;
		}
		...
		public void onSharedObjectClear(ISharedObjectBase so) {		
			log.info("onSharedObjectClear path: {}", path);
		}

		public void onSharedObjectConnect(ISharedObjectBase so) {		
			log.info("onSharedObjectConnect path: {}", path);
		}

		public void onSharedObjectDelete(ISharedObjectBase so, String key) {		
			log.info("onSharedObjectDelete path: {} key: {}", path, key);
		}

		public void onSharedObjectDisconnect(ISharedObjectBase so) {		
			log.info("onSharedObjectDisconnect path: {}", path);
		}

		public void onSharedObjectSend(ISharedObjectBase so, String method, List<?> attributes) {
			log.info("onSharedObjectSend path: {} - method: {} {}", path, method, attributes);
		}

		public void onSharedObjectUpdate(ISharedObjectBase so, IAttributeStore attributes) {	
			log.info("onSharedObjectUpdate path: {} - {}", path, attributes);	
		}
		...
		public void onSharedObjectUpdate(ISharedObjectBase so, String key, Object value) {
			log.info("onSharedObjectUpdate path: {} - {} = {}", path, key, value);
